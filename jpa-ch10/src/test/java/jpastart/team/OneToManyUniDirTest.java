...
public class OneToManyUniDirTest extends JpaTestBase {
    ...
    public void find() throws Exception {
        ...
        assertPlayer(t1.getPlayers(), Arrays.asList(data("P1", "선수1"), data("P2", "선수2")));
    }
...
//            em.persist(p5);
    ...
    public void remove() throws Exception {
        ...
        try {
            ...
            Team t1 = em.find(Team.class, "T1");
            Optional<Player> pOpt = t1.getPlayers().stream().filter(p -> p.getId().equals("P2")).findFirst();
            pOpt.ifPresent(p -> t1.removePlayer(p) );
            em.getTransaction().commit();
        } catch(Exception ex) {
            em.getTransaction().rollback();
            throw ex;
